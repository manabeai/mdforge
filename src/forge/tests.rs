...
use crate::{ArgType, BlockNode, ErrorCode, InlineExt, VElement, VNode};
...
fn parse_unclosed_block_returns_error() {
    ...
    assert!(diagnostics
        .iter()
        .any(|d| d.code == ErrorCode::BlockNotClosed));
}
...
fn validate_reports_unknown_and_missing_args() {
    ...
    assert!(diagnostics
        .iter()
        .any(|d| d.code == ErrorCode::MissingRequiredArg));
    assert!(diagnostics.iter().any(|d| d.code == ErrorCode::UnknownArg));
}
...
fn validate_static_enum_and_eval_dynamic_enum() {
    ...
    assert!(validate_errors
        .iter()
        .any(|d| d.code == ErrorCode::InvalidStaticEnumValue));
    ...
    assert!(eval_errors
        .iter()
        .any(|d| d.code == ErrorCode::InvalidDynamicEnumValue));
}
